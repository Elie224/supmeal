# Manuel utilisateur SUPMEAL

Bienvenue dans SUPMEAL ! Ce guide vous accompagne pour utiliser toutes les fonctionnalites de l application.

## Sommaire

1. [Premiers pas](./01-premiers-pas.md)
2. [Gerer ses recettes](./02-recettes.md)
3. [Cookbooks partages](./03-cookbooks.md)
4. [Planifier ses repas](./04-planning.md)
5. [Importer / Exporter](./05-import-export.md)
6. [Suggestions de recettes](./06-suggestions.md)
