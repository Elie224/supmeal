...
export default function SettingsPage() {
  ...
  const handleImport = async (file: File, format: "json" | "csv") => {
    ...
    try {
      ...
      const importedCount = Number(data?.imported_recipes ?? 0);
      setImportMsg(`Succes: ${file.name} deja importe (${importedCount} recette(s) ajoutee(s)).`);
      setMsg(null);
    } catch (e: any) {
      setImportError(e?.response?.data?.detail || "Echec de l import. Verifiez le format du fichier.");
    }
  };
      ...
      <section className="card p-5 space-y-3">
        ...
        <form onSubmit={(e) => { e.preventDefault(); changePwd.mutate(); }} className="space-y-3">
          ...
          <div>
            ...
            <input type="password" autoComplete="current-password" className="input" value={pwd.current} onChange={(e) => setPwd({ ...pwd, current: e.target.value })} />
          </div>
          <div>
            <label className="label">Nouveau mot de passe</label>
            <input type="password" autoComplete="new-password" className="input" minLength={8} value={pwd.next} onChange={(e) => setPwd({ ...pwd, next: e.target.value })} />
          </div>
          ...
          {enabledProviders.length > 0 ? (
            <div className="flex gap-2">
              {enabledProviders.map((provider) => (
                <a key={provider.key} href={oauthUrl(provider.key)} className="btn-outline text-xs">
                  {`Lier ${provider.label}`}
                </a>
              ))}
            </div>
          ) : (
            <p className="text-xs text-charcoal-500">Aucun provider OAuth configure.</p>
          )}
        </div>
      </section>
